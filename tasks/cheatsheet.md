...
и `namespaces`. 
3. Docker можно установить под любую распространенную операционную систему, но для Windows или macOS требуется использования системы виртуализации, в которой запускается ядро Linux, что может  сказаться на производительности и использовании ресурсов.
4. C помощью Docker удобно автоматизировать процесс развертывания приложений и запуска в изолированном окружении.
5. `Образ` (`image`) - это неизменяемый образ файловой системы и набор вспомогательной мета-информации. Образы могут состоять из слоев - срезов файловой системы, которые накладываются друг на друга. Например, образы двух разных версий Python могут иметь один общий слой - базовый набор библиотек операционной системы (см. OverlayFS).  
6. Готовый образ можно `импортировать` на локальную из удаленного реестра.
7. Образ можно собрать локально, с помощью инструкций в `Dockerfile`.
8. Контейнер - процесс, который запускается в изолированном окружении на основе образа. При запуске создается слой, который накладывается на образ. Контейнер можно запустить, остановить, перезапустить, удалить (в этом случае все данные будут потеряны), создать на основе работающего контейнера новый образ.
...
преобразуется в байт-код  (CPython 3.11)
